...
@app.post('/api/task-center/incidents/{incident_id}/resolve', dependencies=[Depends(require_internal_token)])
async def resolve_incident(incident_id: str, app_state: AppStateDep):
	try:
		return app_state.task_center_service.resolve_incident(incident_id)
...
@app.post('/api/task-center/sites/{site_id}/probe', dependencies=[Depends(require_internal_token)])
async def probe_site(site_id: str, app_state: AppStateDep):
	try:
		return await app_state.task_center_service.probe_site(site_id)
	except KeyError as exc:
		raise HTTPException(status_code=404, detail='Site not found') from exc


@app.post('/api/task-center/accounts/{account_id}/preflight', dependencies=[Depends(require_internal_token)])
async def preflight_account(account_id: str, app_state: AppStateDep):
	try:
		return await app_state.task_center_service.preflight_account(account_id)
	except KeyError as exc:
		raise HTTPException(status_code=404, detail='Account or site not found') from exc
