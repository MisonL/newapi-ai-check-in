...
class MemoryStorage(StorageBackend):
	...
	def save_daily_task(self, task: DailyTaskRecord) -> None:
		duplicate_ids = [
			item.id
			for item in self._daily_tasks.values()
	...
	def save_checkin_result(self, result: CheckinResultRecord) -> None:
		duplicate_ids = [
			item.id
			for item in self._checkin_results.values()
			if item.task_id == result.task_id and item.id != result.id
		]
		for duplicate_id in duplicate_ids:
		...
		if incident.dedupe_key:
			duplicate = next(
				(
					...
					if item.dedupe_key == incident.dedupe_key and item.id != incident.id
				),
				None,
			)
