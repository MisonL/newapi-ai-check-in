...
def _seed_storage():
    storage = MemoryStorage(Path('tmp-artifacts'))
...
def test_task_center_executor_persists_success_result():
    storage, site, account, task = _seed_storage()
...
def test_task_center_executor_persists_blocked_incident():
    storage, _, account, task = _seed_storage()
    service = FakeCheckinService(
        NewApiTaskExecutionResult(
            task_status='blocked',
