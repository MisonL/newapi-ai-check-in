...
- 代码、重构和缺陷修复必须遵守 TDD：先写失败测试，确认失败原因，再写最小实现并重新验证。
- 临时截图、审查记录、Playwright 输出、缓存和本地数据库不进入仓库；需要保留的设计文档放在 `docs/architecture/` 或 `docs/superpowers/`。
